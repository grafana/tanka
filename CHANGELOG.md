...
This release contains these enhancements:

 * **cli** Allow `tk env add` to use `--server-from-context ([#183](https://github.com/grafana/tanka/issues/183))
 ...
 * **jsonnet** Added 'vendor overrides'. With this, if we want to override a vendored library for just one environment,
 ...
 * **kubernetes** Add --validate flag to tk apply, to allow invoking kubectl without schema validation (kubectl apply --validate=false)
