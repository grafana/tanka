...
const analyzer = new _static.Analyzer(documentManager, compiler);
const reportDiagnostics = (doc) => {
...
connection.onInitialize((params) => exports.initializer(docs, params));
connection.onDidChangeConfiguration(params => exports.configUpdateProvider(params));
...
connection.onCompletion(position => {
    return analyzer
        ...
        .then(completions => completions.map(completionInfoToCompletionItem));
});
...
connection.listen();
exports.initializer = (documents, params) => {
    return {
        capabilities: {
...
exports.configUpdateProvider = (change) => {
    if (change.settings == null || change.settings.jsonnet == null ||
    ...
    libResolver.libPaths = im.List(jsonnet.libPaths);
};
const positionToLocation = (posParams) => {
    return new lexical.Location(posParams.position.line + 1, posParams.position.character + 1);
};
const completionInfoToCompletionItem = (completionInfo) => {
    let kindMapping;
    switch (completionInfo.kind) {
        ...
        default: throw new Error(`Unrecognized completion type '${completionInfo.kind}'`);
    }
    ...
    const completionItem = completionInfo;
