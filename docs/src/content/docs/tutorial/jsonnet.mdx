...
:::tip[Bonus]
There is also `tk eval`, which displays the raw JSON object
...
:::note
Tanka won't create the namespace for you -- you need to include it in
...
:::note
Diff may fail before the first apply when the namespace does not yet
...
See the [tk apply](/reference/tk-apply/) reference for a full list of flags.
