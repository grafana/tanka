...
// ignoredManifest builds on orphanedManifest, additionally carrying the
// prune-ignore annotation.
func ignoredManifest(kind, name, uid string) manifest.Manifest {
	m := orphanedManifest(kind, name, uid)
	m.Metadata().Annotations()[process.AnnotationPruneIgnore] = "true"
	return m
}

// ownedManifest builds on orphanedManifest, additionally carrying a
// controller ownerReference back to a parent object, e.g. a Secret generated
// by external-secrets from an ExternalSecret.
