...
func NewGitHubTools(repoRoot string) ([]adktool.Tool, error) {
	ght := &GitHubTools{repoRoot: repoRoot}
	var tools []adktool.Tool
	for _, mk := range []func() (adktool.Tool, error){
		ght.pushTool, ght.prCreateTool, ght.prListTool,
	} {
		t, err := mk()
		if err != nil {
			return nil, err
...
func (ght *GitHubTools) pushTool() (adktool.Tool, error) {
	return functiontool.New(
		functiontool.Config{
			...
			InputSchema: mustSchema(`{
				"type": "object",
				"properties": {
					"branch": {"type": "string", "description": "Name of the local branch to push"}
				},
				"required": ["branch"]
			}`),
		},
		func(ctx adktool.Context, input map[string]any) (map[string]any, error) {
			...
			token := os.Getenv("GITHUB_TOKEN")
			if token == "" {
				return nil, fmt.Errorf("GITHUB_TOKEN environment variable is required to push")
			}
			r, err := gogit.PlainOpen(ght.repoRoot)
			if err != nil {
			...
			if err == gogit.NoErrAlreadyUpToDate {
				return map[string]any{"output": fmt.Sprintf("Branch %q is already up to date on origin", params.Branch)}, nil
			}
			if err != nil {
				return nil, fmt.Errorf("pushing branch %q: %w", params.Branch, err)
			}
			return map[string]any{"output": fmt.Sprintf("Successfully pushed branch %q to origin", params.Branch)}, nil
		})
}

func (ght *GitHubTools) prCreateTool() (adktool.Tool, error) {
	return functiontool.New(
		functiontool.Config{
			...
			InputSchema: mustSchema(`{
				"type": "object",
				"properties": {
					"title": {"type": "string", "description": "Pull request title (keep it under 70 characters)"},
					...
					"base_branch": {"type": "string", "description": "The branch to merge into (e.g. 'main')", "default": "main"}
				},
		...
		func(ctx adktool.Context, input map[string]any) (map[string]any, error) {
			...
			return map[string]any{"output": fmt.Sprintf("Created PR #%d: %s\nURL: %s", pr.GetNumber(), pr.GetTitle(), pr.GetHTMLURL())}, nil
		})
}

func (ght *GitHubTools) prListTool() (adktool.Tool, error) {
	return functiontool.New(
		functiontool.Config{
			Name:        "github_pr_list",
		...
		func(ctx adktool.Context, _ map[string]any) (map[string]any, error) {
			client, err := githubClient()
			if err != nil {
				return nil, err
			}
			...
			if len(prs) == 0 {
				return map[string]any{"output": "No open pull requests"}, nil
			}
